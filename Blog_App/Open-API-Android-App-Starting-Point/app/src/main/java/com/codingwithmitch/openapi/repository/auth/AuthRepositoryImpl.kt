...
import com.codingwithmitch.openapi.util.ErrorHandler.Companion.ERROR_SAVE_ACCOUNT_PROPERTIES
...
import com.codingwithmitch.openapi.util.ErrorHandler.Companion.INVALID_CREDENTIALS
...
constructor(
    ...
    override fun attemptLogin(
        ...
        if (loginFieldError.equals(LoginFields.LoginError.none())) {
            ...
            Log.d(TAG, "emitting error: ${loginFieldError}")
            emit(
                buildError(
                    loginFieldError,
                    UIComponentType.Dialog(),
    ...
    override fun checkPreviousAuthUser(stateEvent: StateEvent): Flow<DataState<AuthViewState>> = flow {
    ...
    override fun returnNoTokenFound(stateEvent: StateEvent): DataState<AuthViewState> {
        Log.d(TAG,"AuthRepositoryImpl : Returning NoTokenFound Message")
        return DataState.error(
            response = Response(
                RESPONSE_CHECK_PREVIOUS_AUTH_USER_DONE,
                UIComponentType.None(),
